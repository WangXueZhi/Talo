...
- 新增项目策略记录和可选桥接功能，把审核后的项目指导同步到 Agent 指令文件。
- 改进 Desktop 项目发现逻辑，支持在 Codex、Claude Code 和 Antigravity 会话之间按 Git
  worktree 与远程仓库归并项目。
- 新增策略状态、更新和桥接生命周期命令，并补充项目级流程的回归测试。

## 0.14.4 - 2026-08-19

- 新增可配置的 `manual` 与 `smart` 记忆审核策略；智能模式会自动提交规模受限、同项目、非推断，
  且不修改或关联既有记忆的提案。
- 推断内容、跨项目来源、既有记忆变更、大批量提案和缺少依据的高影响内容仍进入统一审核箱，
  在减少打断的同时保留关键安全检查。
- 新增 `review-policy` CLI 命令、桌面端记忆保存策略设置、结构化 `autoReview` 提案结果、
  Codex 窄权限规则和回归测试。
