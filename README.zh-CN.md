...
重要工作结束时，Agent 会提出值得长期保留的结论、流程、风险、证据和下一步。默认人工模式会
逐次审核；可在桌面设置中启用“智能保存”，让低风险、同项目、证据充分的新记忆自动写入，
而推断内容、跨项目引用、已有记忆修改和高影响无依据内容仍进入审核箱。
...
- **统一保存策略。** 所有平台都可以提出候选；智能策略自动处理低风险内容，其余统一进入审核箱。
